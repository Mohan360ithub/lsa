...
recurring_items=frappe.get_all('Item', 
                                    filters= {
                                                "custom_service_master": ['not in', [None]],
                                            },
                                    fields= ['name', 'custom_service_master'],
                                )

addon_master_dict={it.name:it.custom_service_master for it in recurring_items}
...
def execute(filters=None):
    ...
    columns = [
        ...
        {"label": "Current Recurring Fees", "fieldname": "service_current_charges", "fieldtype": "Currency", "width": 100},
        {"label": "Frequency", "fieldname": "frequency", "fieldtype": "Data", "width": 50},
        {"label": "Annual Fees", "fieldname": "service_annual_fees", "fieldtype": "Currency", "width": 100},
        ...
        {"label": "Completion Status", "fieldname": "completion_status", "fieldtype": "Data", "width": 100},
        {"label": "Month Cover Status", "fieldname": "month_cover_status", "fieldtype": "Data", "width": 100},
...
def get_data(filters):
    ...
    soi_list = frappe.get_all("Sales Order Item", filters={"docstatus": ("not in", [2]), 
                                                           ...
                                                           fields=["name", "parent", "item_code", "rate", "description", "qty","custom_soi_from_date","custom_soi_to_date","custom_service_master"], 
    ...
    for soi in soi_list:
        if soi.parent in so_dict:
            ...
            item_key = soi.custom_service_master
    ...
    # print(soi_qty_map["ANITA DEVI-GCXPD0597L"])
    data = []
    

    service_addons = frappe.get_all("Service Master Addon",
                                    fields=["addon_service_name","current_charges","frequency","parent"], 
                                    # filters={"parenttype":service.name},
                                    )
    service_addons_map = {}
    for addon in service_addons:
        if addon.parent not in service_addons_map:
            service_addons_map[addon.parent] = {}
        service_addons_map[addon.parent][addon.addon_service_name] = addon
        # if addon.addon_service_name not in service_addons_map[addon.parent]:
        #     service_addons_map[addon.parent][addon.addon_service_name] = [addon]
        # else:
        #     service_addons_map[addon.parent][addon.addon_service_name] += [addon]
        ...
        if True:
            for customer_service in customer_services:
                ...
                for service_addon in service_addons_map[customer_service.name]:
                    ...
                    service_frequency=service_addons_map[customer_service.name][service_addon]["frequency"]
                    yearly_frequency_count=frequency_map[service_frequency]
                    service_annual_fees=service_current_charges*yearly_frequency_count
                    ...
                    addon_customer_service.update(custome_map_value)
                    addon_customer_service.update({
                        "service_annual_fees":service_annual_fees,
                        "service_current_charges":service_current_charges,
                        "frequency":service_frequency,
                        "file_type":service.name,
                        "file_name":customer_service[master_service_fields[service.name][1][2]],
                        "service_name":service_addon,
                        ...
                        "month_cover_status": "Complete" if not missing_months else "Incomplete",
                        "sales_order_count": sales_order_count,
                        "sales_order_details": sales_order_details,

                    })

                    data.append(addon_customer_service)
...
##################### Old Report tracing based on only SO Item code instead of service master reference ##############################
# master_service_fields = {
#         "Gstfile": ["gstfile", ["name", "customer_id", "company_name", "gst_number", "gst_user_name", "gst_password", "current_recurring_fees", "frequency", "annual_fees", "executive_name", "service_name", "description", "frequency","enabled"]],
#         "IT Assessee File": ["it-assessee-file", ["name", "customer_id", "assessee_name", "pan", "it_password", "current_recurring_fees", "frequency", "annual_fees", "executive_name", "service_name", "description", "frequency","enabled"]],
#         "MCA ROC File": ["mca-roc-file", ["name", "customer_id", "company_name", "cin", "trace_user_id", "trace_password", "current_recurring_fees", "frequency", "annual_fees", "executive_name", "service_name", "description", "frequency"]],
